...
class StableDiffusionPipelineVideo2VideoMaskC(DiffusionPipeline):
    ...
    def __init__(self, vae: AutoencoderKL, unet: UNet2DConditionModel,
        ...
        self.scheduler_pre = scheduler_pre
    ...
    def outpainting_with_random_masked_latent_inference_bidirection(
        ...
        # latents = noise
        if use_add_noise:
            latents = self.scheduler_pre.add_noise(
                ori_latents, noise, timesteps)
        ...
        for i, t in enumerate(timesteps):
            ...
            if do_classifier_free_guidance and not do_previous_guidance:
                ...
                noise_pred = noise_pred_uncond_local_uncond_global + guidance_scale * (noise_pred_cond_local_uncond_global - noise_pred_uncond_local_uncond_global) + \
                    previous_guidance_scale * \
                    (noise_pred_cond_local_cond_global -
                     noise_pred_cond_local_uncond_global)
    ...
    # outpainting_with_random_masked_latent_inference_bidirectionとは違い、1度だけノイズ除去を行う
    def outpainting_with_random_masked_one_step(
            self,
            ori_image,  # 元の入力動画フレーム
            mask_image,  # マスク処理された入力画像
            mask,  # バイナリマスク（1:生成する領域、0:保持する領域）
            generator,  # 乱数生成器（再現性のため）
            fps,  # フレームレート
            t: Optional[torch.LongTensor] = None,  # 時間ステップ
            use_add_noise=True,  # 元の画像にノイズを追加するかどうか
            strength: float = 1.0,  # ノイズ強度（0-1）
            batch_size: int = 1,  # バッチサイズ
            num_frames: int = 16,  # フレーム数
            num_global_frames: int = 16,  # グローバルフレーム数（時間的一貫性のため）
            height: int = 256,  # 画像の高さ
            width: int = 256,  # 画像の幅
            guidance_scale: float = 2.0,  # クラスフリーガイダンスのスケール
            previous_guidance_scale: float = 1.0,  # 前フレームガイダンスのスケール
            num_inference_steps: int = 50,  # 推論ステップ数
            eta: float = 0.0,  # DDIMスケジューラのEtaパラメータ
            latents: Optional[torch.FloatTensor] = None,  # 事前生成された潜在変数
            latents_dtype=torch.float16,  # 潜在変数のデータ型
            output_type: Optional[str] = "pil",  # 出力形式
            callback: Optional[Callable[[int, int, torch.FloatTensor],
                                        None]] = None,  # コールバック関数
            callback_steps: Optional[int] = 1,  # コールバック間隔
            # mode = 'dense',  # 未使用パラメータ
            already_outpainted_latents=None,  # すでにアウトペイントされた潜在変数
            copy_already_frame=False,  # 既にアウトペイントされたフレームをコピーするかどうか
            replace_gt=False,  # Ground Truthで置き換えるかどうか
            global_frames=None,  # 時間的一貫性のためのグローバルフレーム
            **kwargs):
        """
        1回のノイズ除去ステップでビデオアウトペインティングを行う関数。
        outpainting_with_random_masked_latent_inference_bidirection関数とは異なり、
        この関数は単一のノイズ除去ステップのみを実行します。
        主に訓練時に使用され、ノイズ予測モデルの学習に役立ちます。
        """
        # 入力画像の高さと幅が8の倍数であることを確認（VAEの要件）
        if height % 8 != 0 or width % 8 != 0:
            raise ValueError(
                f"`height` and `width` have to be divisible by 8 but are {height} and {width}."
            )

        # strengthパラメータが正しい範囲内であることを確認
        if strength < 0 or strength > 1:
            raise ValueError(
                f"The value of strength should in [0.0, 1.0] but is {strength}"
            )
        ...
        # 潜在空間の形状を設定（batch, channels, frames, height/8, width/8）
        latents_shape = (batch_size, 4, num_frames, height // 8, width // 8)
        b, c, f, h, w = latents_shape
        ...
        # マスクを潜在空間のサイズに合わせてリサイズ
        mask = F.interpolate(mask, size=(h, w)).view((b, 1, f, h, w))

        # 元画像、マスク画像、条件なし画像をVAEでエンコード
        with torch.no_grad():
            res_latent = self.vae.encode(
                torch.cat([
                    init_images, mask_image,
        ...
        for idx, already_outpainted_latent in enumerate(
                already_outpainted_latents):
            if already_outpainted_latent is not None:
                masked_latents[:, :, idx] = already_outpainted_latent.to(
                    masked_latents.device)
                mask[:, :, idx] = torch.zeros((h, w)).to(mask.device)
                uncond_mask[:, :, idx] = torch.zeros((h, w)).to(mask.device)
                ...
                frame_already_outpainted_list.append('|')
        # print(' '.join(frame_already_outpainted_list))  # アウトペイント状況を表示
        ...
        # 推論ループ用の timesteps 列を準備（PNDM などに渡す）
        t_start = max(num_inference_steps - (timesteps[0].item()) + 0, 0)
        infer_timesteps = self.scheduler.timesteps[t_start:].to(self.device)

        # スケジューラに必要な追加パラメータを準備
        accepts_eta = "eta" in set(
            inspect.signature(self.scheduler.step).parameters.keys())
        extra_step_kwargs = {}
        if accepts_eta:
        ...
        if global_frames is not None:
            ...
            global_frames = torch.zeros((num_global_frames, 4,
                                         height // 8, width // 8),
        ...
        do_classfier_num = 4 if do_previous_guidance else 1
        noise_pred = self.unet(latent_model_input,
                               t,
                               fps=fps,
                               encoder_hidden_states=global_frames,
                               ...
                               g=num_global_frames,
                               is_empty_img=False,
                               do_classfier=do_classfier_num).sample
