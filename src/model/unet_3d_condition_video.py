...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def _set_gradient_checkpointing(self, enable=True, gradient_checkpointing_func=torch.utils.checkpoint.checkpoint):
        for block in self.down_blocks:
            # Added DownBlock2D for completeness if used
            if isinstance(block, (CrossAttnDownBlock3D, DownBlockP3D, DownBlock2D)):
                if hasattr(block, 'gradient_checkpointing'):
                    block.gradient_checkpointing = enable
                # It's also common for blocks to have a method like set_gradient_checkpointing
                elif hasattr(block, 'set_gradient_checkpointing'):
                    block.set_gradient_checkpointing(enable)

        if self.mid_block is not None:
            # Added UNetMidBlock2DCrossAttn for completeness
            if isinstance(self.mid_block, (UNetMidBlock3DCrossAttn, UNetMidBlock2DCrossAttn)):
                if hasattr(self.mid_block, 'gradient_checkpointing'):
                    self.mid_block.gradient_checkpointing = enable
                elif hasattr(self.mid_block, 'set_gradient_checkpointing'):
                    self.mid_block.set_gradient_checkpointing(enable)

        for block in self.up_blocks:
            # Added UpBlock2D for completeness
            if isinstance(block, (CrossAttnUpBlock3D, UpBlockP3D, UpBlock2D)):
                if hasattr(block, 'gradient_checkpointing'):
                    block.gradient_checkpointing = enable
                elif hasattr(block, 'set_gradient_checkpointing'):
                    block.set_gradient_checkpointing(enable)
